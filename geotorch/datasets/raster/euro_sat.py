...
import pandas as pd
from .utility import textural_features as ttf
from .utility import spectral_indices as si
...
class EuroSATDataset(Dataset):
	...
	_idx_to_class = {i:j for i, j in enumerate(EURO_SAT_CLASSES)}
	_class_to_idx = {value:key for key, value in _idx_to_class.items()}
	_rgb_band_indices = torch.tensor([3, 2, 1])
	...
	def __init__(self, root = "dataset", download = False, bands = SPECTRAL_BANDS, include_additional_features = False, additional_features_list = None,  transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
		super().__init__()
