...
class SpatiotemporalDfToTorchData(Dataset):
    '''
    This Class creates a grid-based spatiotemporal PyTorch dataset from a grid-based spatiotemporal dataframe
    that is created through GeoTorch Preprocessing module
    ...
    def __init__(self, df_st, temporal_id, spatial_id, columns_list, temporal_length, height, width, missing_data = 0, lead_time = 2*24, normalize=True):
        super().__init__()

        st_array = np.empty((temporal_length, len(columns_list), height, width))
        st_array[:] = missing_data

        for row in df_st.collect():
            temporal_pos = row[temporal_id]
            spatial_pos = row[spatial_id]
            h = spatial_pos // width
            w = spatial_pos % width
            for i in range(len(columns_list)):
                st_array[temporal_pos][i][h][w] = row[columns_list[i]]

        self.full_data = st_array
        self.normalize = normalize

        max_data = np.max(self.full_data)
        min_data = np.min(self.full_data)
        self.min_max_diff = max_data - min_data
        self.min_max_sum = max_data + min_data
        if normalize:
    ...
    def set_periodical_representation(self, len_closeness=3, len_period=4, len_trend=4, T_closeness=1, T_period=24,
                                      T_trend=24 * 7):
        '''
    ...
    def _create_feature_vector(self, all_data, len_closeness, len_period, len_trend, T_closeness, T_period,
                               T_trend):
        ...
        if len_closeness > 0:
            ...
            for i in range(len_closeness - 1):
                self.X_closeness = np.concatenate((self.X_closeness, all_data[number_of_skip_hours - T_closeness * (
                            2 + i):len_total - T_closeness * (2 + i)]), axis=1)
        if len_period > 0:
            self.X_period = all_data[number_of_skip_hours - T_period:len_total - T_period]
            for i in range(len_period - 1):
                self.X_period = np.concatenate((self.X_period, all_data[number_of_skip_hours - T_period * (
        ...
        if len_trend > 0:
            ...
            for i in range(len_trend - 1):
                self.X_trend = np.concatenate((self.X_trend, all_data[number_of_skip_hours - T_trend * (
